...
func main() {
	...
	var leftovers commands.CleanupLeftovers
	switch appConfig.State.IAAS {
	case "aws":
		leftovers = commands.NewCleanupLeftovers("aws", logger, appConfig.State.AWS.AccessKeyID, appConfig.State.AWS.SecretAccessKey, appConfig.State.AWS.Region)
	case "azure":
		leftovers = commands.NewCleanupLeftovers("azure", logger, appConfig.State.Azure.ClientID, appConfig.State.Azure.ClientSecret, appConfig.State.Azure.SubscriptionID, appConfig.State.Azure.TenantID)
	case "gcp":
		leftovers = commands.NewCleanupLeftovers("gcp", logger, appConfig.State.GCP.ServiceAccountKeyPath)
	}
	if err != nil {
		log.Fatalf("\n\n%s\n", err)
	}
	...
	commandSet["cleanup-leftovers"] = leftovers
